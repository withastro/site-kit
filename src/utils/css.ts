...
function isCustomPropertyShorthand(candidate: string): candidate is `--${NonEmptyString}` {
  return /^--\D{1,100}/.test(candidate)
}
...
export function parseCssLength(value: string | number | undefined | null): CSSLength | undefined {
  ...
  return typeof value === 'number'
    ? value >= 0
      ...
      : undefined
    : isCssLength(value)
      ? isCustomPropertyShorthand(value) ? `var(${value})` : value
      : undefined
}